...
class QuadSusp(optics.OpticalCouplerBase):
    ...
    def Pend3(self):
        k, theta = pendulum_k(
            ...
            length_m  = self.l3 + self.l2 + self.l1 + self.ln,
            r_fiber_m = self.r3,
            Ymod      = self.Y3,
            theta     = self.theta3,
        )
    ...
    def __build__(self):
        try:
            ...
            self.Pend3.A.bond(self.Platform.A)
...
class LIGODetectorOuter(base.SystemElementBase):
    def __init__(self, **kwargs):
        super(LIGODetectorOuter, self).__init__(**kwargs)
        ...
        self.my.EX = optics.Mirror(
            T_hr = 1e-6,
            L_hr = 10e-6 if not self.lossless else 0,
            #facing_cardinal = 'W',
        ...
        if False and not self.missing_PR:
...
class LIGODetectorInner(base.SystemElementBase):
    ligo_outer = None

    def __init__(self, **kwargs):
        super(LIGODetectorInner, self).__init__(**kwargs)
        ...
        base.OOA_ASSIGN(self).lPRC_m  = 1   # PRCL: lPRC_m = lPR + (lIX + lIY) / 2
        base.OOA_ASSIGN(self).lSRC_m  = 1   # SRCL: lSRC_m = lSR + (lIX + lIY) / 2
        base.OOA_ASSIGN(self).lasy_m  = 0.0504   # Schnupp Asy: lasy_m = lIX - lIY
        base.OOA_ASSIGN(self).lmean_m = 1   # (lIX + lIY) / 2
        base.OOA_ASSIGN(self).larm_m  = 1   # (lIX + lIY) / 2
        ...
        base.OOA_ASSIGN(self).lBS_PR_m  = self.lPRC_m - self.lmean_m      # distance from PR to BS
        base.OOA_ASSIGN(self).lBS_SR_m  = self.lSRC_m - self.lmean_m      # distance from SR to BS
        base.OOA_ASSIGN(self).lPR_PR2_m = 0                               # distance from PR to PR2
        base.OOA_ASSIGN(self).lPR2_BS_m = self.lBS_PR_m - self.lPR_PR2_m  # distance from PR2 to BS
        ...
        self.ligo_outer.IX.Z.bond(self.EX.Z)
        self.my.IY_sus = QuadSusp()
        self.my.IY = optics.Mirror(
            T_hr = .1e-3,
            L_hr = 0 if not self.lossless else 0,
        ...
        self.ligo_outer.IY.Z.bond(self.EY.Z)
        if False and not self.missing_PR:
            self.my.PR = optics.Mirror(
                T_hr = 30e-3,
                L_hr = 37.5e-6 if not self.lossless else 0,
...
class LIGOBasicOperation(base.SystemElementBase):
    def __init__(self, **kwargs):
        ...
        self.my.LIGO_outer = LIGODetectorOuter()
        self.my.input_outer  = LIGOInputBasic()
        self.my.output_outer = LIGOOutputHomodyne(
            LIGO_obj  = self.LIGO_outer,
            input_obj = self.input_outer,
        )

        self.system.bond_sequence(
            self.input_outer.INPUT_ATTACH_POINT,
            self.LIGO_outer.INPUT_ATTACH_POINT,
        )

        self.system.bond_sequence(
            self.LIGO_outer.OUTPUT_ATTACH_POINT,
            self.output_outer.OUTPUT_ATTACH_POINT,
        )


        self.my.LIGO_inner = LIGODetectorInner(
            ligo_outer = self.LIGO_outer,
        )
        self.my.input_inner  = LIGOInputBasic()

        #self.output = LIGOOutputBasic(LIGO_obj = self.LIGO)
        self.my.output_inner = LIGOOutputHomodyne(
            LIGO_obj  = self.LIGO_outer,
            input_obj = self.input_inner,
        )

        self.system.bond_sequence(
            self.input_inner.INPUT_ATTACH_POINT,
            self.LIGO_inner.INPUT_ATTACH_POINT,
        )

        self.system.bond_sequence(
            self.LIGO_inner.OUTPUT_ATTACH_POINT,
            self.output_inner.OUTPUT_ATTACH_POINT,
        )
